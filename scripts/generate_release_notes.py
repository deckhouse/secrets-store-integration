...
def latest_changelog_version_string(en_files: List[str]) -> Optional[str]:
    """Highest semver among English changelog files, as 'X.Y.Z' (no v prefix)."""
    if not en_files:
        return None
    last_path = en_files[-1]
    raw = parse_version_from_filename(os.path.basename(last_path))
    if raw.startswith("v"):
        raw = raw[1:]
    return raw


def read_chart_version(project_root: Path) -> Optional[str]:
    path = project_root / "Chart.yaml"
    if not path.is_file():
        return None
    ...
    if chart_version is None:
        return None
    return str(chart_version).strip()


def check_changelog_matches_version_file(
    en_files: List[str], project_root: Path
) -> Tuple[bool, str]:
    """
    Returns (ok, message). When ok is False, message is an error for stderr.
    """
    changelog_ver_str = latest_changelog_version_string(en_files)
    if changelog_ver_str is None:
        return False, "ERROR: Could not determine latest version from CHANGELOG."

    chart_ver_str = read_chart_version(project_root)
    if chart_ver_str is None:
        p = project_root / "Chart.yaml"
        return (
            False,
            f"ERROR: Chart version missing or unreadable: {p}",
        )

    try:
        changelog_v = version.parse(changelog_ver_str)
        chart_v = version.parse(chart_ver_str)
    except version.InvalidVersion as e:
        return False, f"ERROR: Invalid version string for comparison: {e}"

    if changelog_v != chart_v:
        vp = project_root / "Chart.yaml"
        return (
            False,
            "ERROR: Latest CHANGELOG version does not match Chart.yaml version.\n"
            f"  CHANGELOG (max, English *.yml): v{changelog_ver_str}\n"
            f"  {vp} version: {chart_ver_str}\n"
            "Update CHANGELOG entries or Chart.yaml so they match.",
        )
