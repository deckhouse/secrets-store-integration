			...
			wantedContainers: []corev1.Container{
				{
					...
					LivenessProbe: &corev1.Probe{
						ProbeHandler: corev1.ProbeHandler{
							Exec: &corev1.ExecAction{
								Command: []string{"/vault/env-injector", "/bin/bash"},
							},
						},
					},
					Env: []corev1.EnvVar{
				...
				containers: []corev1.Container{
					{
						...
						Env: []corev1.EnvVar{
							{
								Name:  "myvar",
								Value: "scheme://${secrets-store:secret/data/account#username}:${secrets-store:secret/data/account#password}@127.0.0.1:8080",
							},
						},
					},
				},
			...
			wantedContainers: []corev1.Container{
				{
					...
					VolumeMounts: []corev1.VolumeMount{{Name: "stronghold-env", MountPath: "/vault/"}},
					Env: []corev1.EnvVar{
						{Name: "myvar", Value: "scheme://${secrets-store:secret/data/account#username}:${secrets-store:secret/data/account#password}@127.0.0.1:8080"},
						{Name: "VAULT_ADDR", Value: "addr"},
						{Name: "VAULT_SKIP_VERIFY", Value: "false"},
						{Name: "VAULT_AUTH_METHOD", Value: "jwt"},
						{Name: "VAULT_PATH", Value: "path"},
