...
Доставить секреты в приложение из совместимого с Vault хранилища можно несколькими способами:
...
1. В хранилище обращается приложение-прослойка, и пользовательское приложение получает доступ к секретам из переменных среды.
   ...
   > Если нет возможности читать из файлов, можно использовать этот вариант, но он небезопасен. При таком подходе секретные данные хранятся в Kubernetes (а также в etcd) и потенциально могут быть прочитаны на любом узле кластера.
...
<td>Два пода на каждый узел (DaemonSet)</td>
...
<td>Один под на каждый узел (DaemonSet)</td>
...
Этот метод интеграции реализуется оператором секретов Kubernetes с набором CRD, отвечающих за синхронизацию секретов из Vault в секреты Kubernetes.

Минусы:

- Секрет находится и в хранилище секретов, и в секрете Kubernetes (доступном через API Kubernetes). Секрет также хранится в etcd и потенциально может быть считан на любом узле кластера или извлечён из резервной копии etcd. Исключить хранение данных в секретах Kubernetes невозможно.
...
- Так как сбор метрик осуществляется с каждого контейнера, с таким подходом мы получим в два раза больше метрик только по контейнерам.
