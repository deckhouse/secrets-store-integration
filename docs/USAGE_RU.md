...
Для автоматической настройки работы модуля secrets-store-integration в связке с модулем [Deckhouse Stronghold](/products/kubernetes-platform/modules/stronghold/stable/) потребуется ранее [включенный](/products/kubernetes-platform/modules/stronghold/stable/usage.html#включение-модуля) и настроенный Stronghold.
