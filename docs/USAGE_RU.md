...
>   * команда с использованием консольной версии Stronghold ([руководство по установке](https://developer.hashicorp.com/vault/docs/install));
...
1. Создать в Stronghold секрет типа kv2 по пути `test-kv/myapp` и поместить туда значения `DB_USER` и `DB_PASS`.
...
3. Создать в Stronghold политику, разрешающую чтение секретов по пути `secret/myapp`.
4. Создать в Stronghold роль `my-namespace_backend` для сервис-аккаунта `myapp` в неймспейсе `my-namespace` и привязать к ней созданную ранее политику.
5. Создать в кластере неймспейс `my-namespace`.
6. Создать в созданном неймспейсе сервис-аккаунт `myapp`.
