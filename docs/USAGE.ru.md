...
Пример конфигурации модуля для использования Vault-совместимого хранилища секретов, запущенного по адресу `stronghold.domain.tld` на TLS-порту по умолчанию (`443`):
...
1. Создайте в Stronghold роль для сервис-аккаунта `myapp-sa` в пространстве имен `myapp-namespace` и привяжите к ней созданную ранее политику.
   ...
   * Повторите то же самое для удаленных кластеров, указав другой путь аутентификации:
   ...
   Эти настройки позволяют любому поду из пространства имен `myapp-namespace` из обоих кластеров Kubernetes, который использует ServiceAccount `myapp-sa`, аутентифицироваться и авторизоваться в Stronghold для чтения секретов согласно политике `myapp-ro-policy`.
...
1. Init-контейнер помещает из служебного образа статически собранный бинарный файл-инжектор в общую для всех контейнеров под временную директорию.
...
| `secrets-store.deckhouse.io/namespace` | Из модуля | Неймспейс, который будет использоваться для подключения к хранилищу |
...
1. Создайте под с именем `myapp1`, который подключит все переменные из хранилища по пути `demo-kv/data/myapp-secret`:
...
1. Проверьте логи пода после его запуска. В результате должны быть выведены все переменные из `demo-kv/data/myapp-secret`:
...
1. Создайте тестовый под с именем `myapp2`, который подключит требуемые переменные из хранилища по шаблону:
...
1. Проверьте логи пода после его запуска. В результате должны быть выведены переменные из `demo-kv/data/myapp-secret`:
...
В этом примере используются сервисный аккаунт `myapp-sa` и неймспейс `myapp-namespace`, созданные на этапе [подготовки тестового окружения](#подготовка-тестового-окружения).
...
1. Создайте в кластере тестовый под с именем `myapp3`, который подключит секрет из хранилища в виде файла:
